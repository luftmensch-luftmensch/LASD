...
//Auxiliary function for specific constructors
template <typename Data>
void BinaryTreeLnk<Data>::Insert(const LinearContainer<Data>& con, BinaryTreeLnk<Data>::NodeLnk*& root, unsigned int index, unsigned long condim){
  if(index<condim){
    root = new NodeLnk(con[index]);
    dim++;
    NodeLnk* node=root;
    Insert(con,node->left ,(2*index)+1,condim);
    Insert(con, node->right,(2*index)+2,condim);
