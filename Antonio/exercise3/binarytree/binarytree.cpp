...
//Auxiliary function for operator == of two trees
template <typename Data>
bool Equals(const typename BinaryTree<Data>::Node& firsttreenode,const typename BinaryTree<Data>::Node& secondtreenode) noexcept {
  ...
  if(firsttreenode.HasLeftChild() && secondtreenode.HasLeftChild()){
    left = Equals<Data>(firsttreenode.LeftChild(), secondtreenode.LeftChild());
  }
                     //Recursive call to the right subtree if the boolean var for the left one is still true
  if(left && firsttreenode.HasRightChild() && secondtreenode.HasRightChild()){
    right = Equals<Data>(firsttreenode.RightChild(), secondtreenode.RightChild());
  }
  return left && right;
}
...
bool BinaryTree<Data>::operator==(const BinaryTree<Data>& binarytree) const noexcept{
                         //If both have the same pointer, returns true
  if(this == &binarytree){
    return true;
  }
                         //If both are empty, returns true
  if(binarytree.dim == 0 && dim == 0){
    return true;
  }
                           //If the two sizes are equal, i proceed with recursive calls to the left or right child
  if(dim==binarytree.dim && Equals<Data>(Root(), binarytree.Root())){
    return true;
  }
  else{
    return false;
