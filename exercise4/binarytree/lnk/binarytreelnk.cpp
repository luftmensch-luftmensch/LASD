...
//Auxiliary1 for copy
template <typename Data>
typename BinaryTreeLnk<Data>::NodeLnk* BinaryTreeLnk<Data>::CopyTree(const BinaryTreeLnk<Data>& oldtree){

  BinaryTreeLnk<Data>::NodeLnk* newtree;
  AuxCopyTree(&newtree, &oldtree.Root());
  return newtree;
}

//Auxiliary2 for copy
template <typename Data>
void BinaryTreeLnk<Data>::AuxCopyTree(typename BinaryTreeLnk<Data>::NodeLnk** newtree,
const typename BinaryTreeLnk<Data>::NodeLnk* oldtreenode){

  (*newtree) = new NodeLnk(oldtreenode->Element());

  if(oldtreenode->HasLeftChild()){
    AuxCopyTree(&(*newtree)->left, &oldtreenode->LeftChild());
  }

  if(oldtreenode->HasRightChild()){
    AuxCopyTree(&(*newtree)->right, &oldtreenode->RightChild());
  }
}
...
template <typename Data>
void BinaryTreeLnk<Data>::AuxRecursiveDeleteTree(typename BinaryTreeLnk<Data>::NodeLnk*& currentnode) noexcept{
